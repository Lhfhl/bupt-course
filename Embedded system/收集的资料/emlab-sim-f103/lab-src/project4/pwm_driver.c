...
static TIM_TypeDef * timmap[] = {
    TIM1, TIM2, TIM3, TIM4
#ifdef STM32F407xx
		, TIM5, TIM6, TIM7, TIM8, TIM9, TIM10, TIM11, TIM12, TIM13, TIM14, 0
#endif
};
...
static int stm32_pwm_open(struct device_t *dev, const char *filename, int flags, int mode) {
    ...
    //TODO:从filename提取定时器号和通道号，编码到pwmcode中，return会传回上层的fd->file_data中
		pwmcode = stm32_pwm_get(filename);
		
		int pwm_num = PWM_PORT(pwmcode);		//定时器号
		int channel = PWM_CHANNEL(pwmcode);			//通道号
	
		TIM_HandleTypeDef tim_handle;
		tim_handle.Channel = channel;						//通道号
		tim_handle.Instance = timmap[pwm_num];		//寄存器基址
	
		HAL_TIM_PWM_Start(&tim_handle, channel);
		
		timmap[pwm_num]->ARR = 1000000 / 262;				
		timmap[pwm_num]->CCR1 = 1000000 / 262 * 0.8;		
...
static int stm32_pwm_write(struct device_t *dev, void* file_data, char *buffer, int size) {
    ...
    //TODO:从file_data解码出定时器号和通道号，从buffer中读出数据，据此启动或者关闭pwm通道
		int pwmcode = (int)file_data;
		int pwm_num = PWM_PORT(pwmcode);
		int channel = PWM_CHANNEL(pwmcode);
	
		TIM_HandleTypeDef tim_handle;
		tim_handle.Channel = channel;
		tim_handle.Instance = timmap[pwm_num];
		
		if(buffer[0] == 1){
			HAL_TIM_PWM_Start(&tim_handle, channel);		//开启pwm通道
		}
		else{
			HAL_TIM_PWM_Stop(&tim_handle, channel);			//关闭pwm通道
		}
    return 1;
}
...
static int stm32_pwm_ioctl(struct device_t *dev, void* file_data, int cmd, void *args) {
    ...
    (void)dev;
		...
		struct pwm_ioctl_msg *msg = (struct pwm_ioctl_msg*)args;
		...
		case IOCMD_PWM_SET:
		{
			stm32_pwm_set(&tim_handle, channel, period, pulse);
		}
		break;
		default:
			ASSERT(0);
