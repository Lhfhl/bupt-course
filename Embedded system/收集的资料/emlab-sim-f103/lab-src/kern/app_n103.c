...
static void check_gpio(void) {
    gpio_write(GPIOC, 6, gpio_read(GPIOC, 0));
    gpio_write(GPIOC, 7, gpio_read(GPIOC, 1));
    gpio_write(GPIOC, 8, gpio_read(GPIOC, 2));
    gpio_write(GPIOC, 9, gpio_read(GPIOC, 3));
}
...
static void blow(void) {
    ...
    gpio_clr(GPIOC, 7);     //led on
    gpio_clr(GPIOC, 8);
    gpio_clr(GPIOC, 9);
    gpio_clr(GPIOC, 6);
    tim_pwm_start(TIM2, 0);   //beep
    task_sleep(500);
    tim_pwm_stop(TIM2, 0);
}
...
static void smoke_task(void) {
    ...
    while(smoking) {
        ...
        task_sleep(1);
        if(!gpio_read(GPIOC, 0) && !gpio_read(GPIOC, 1))  { //k3+k4
            smoking = 0;
